...
#
# Images produced:
#   ghcr.io/cloudiy-cloud/worker-sdxl:latest   (+ :<git-sha>)  GPU  — image endpoints
#   ghcr.io/cloudiy-cloud/worker-ltx:latest    (+ :<git-sha>)  GPU  — video endpoints
#   ghcr.io/cloudiy-cloud/worker-tts:latest    (+ :<git-sha>)  CPU  — chatterbox (TTS)
#   ghcr.io/cloudiy-cloud/worker-audio:latest  (+ :<git-sha>)  CPU  — stable-audio
#
...
REGISTRY="ghcr.io/cloudiy-cloud"
