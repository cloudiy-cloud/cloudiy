...
Run a local node first:  cloudiy share --token demo
Then:                    python examples/agent_tool.py
"""

import json

from cloudiy_sdk import CloudiyClient, PaymentRequired, as_tool_schema

NODE = "127.0.0.1:8080"
client = CloudiyClient(NODE, token="demo")
...
def dispatch_tool_call(name: str, arguments: dict) -> str:
    if name == "cloudiy_gpu_run":
        return cloudiy_gpu_run(**arguments)
...
if __name__ == "__main__":
    ...
    print("Agent calls cloudiy_gpu_run(vector_add, '1,2,3;4,5,6'):")
    print(" →", cloudiy_gpu_run("vector_add", "1,2,3;4,5,6"))

    print("Agent calls cloudiy_gpu_run(matrix_mul, '2,2,2;1,2,3,4;5,6,7,8'):")
    print(" →", cloudiy_gpu_run("matrix_mul", "2,2,2;1,2,3,4;5,6,7,8"))
